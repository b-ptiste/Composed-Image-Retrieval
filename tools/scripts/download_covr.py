...
def get_urls(split):
    if split == "train":
        ...
        url = (
            "http://imagine.enpc.fr/~ventural/covr/dataset/webvid8m-covr_paths-val.json"
        )
    elif split == "test":
...
def main(args):
    ...
    for i in tqdm(range(0, len(path2url), 1000)):
        path2url_batch = {
            path: path2url[path] for path in list(path2url.keys())[i : i + 1000]
        }
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=args.processes
        ) as executor:
            {
                executor.submit(request_save, url, video_dir / path)
                for path, url in path2url_batch.items()
            }
